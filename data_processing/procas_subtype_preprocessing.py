...
if csf:
    ...
    recurrence_dir = '/mnt/iusers01/gb01/mbaxrap7/scratch/breast_imaging_ML/csv_data'
else:
    ...
    recurrence_dir = 'C:/Users/adam_/PycharmProjects/breast_imaging_ML/csv_data'

pilot_csv_name = '_vendors_grouped_pilot_50subjects_rearranged.csv'
reading_csv_name = '_vendors_grouped_Reader_1704subjects.csv'
recurrence_csv_name = 'Medici_full_formatted_labels.csv'
save_name = 'medici_classification_preprocessed_data.pth'

pilot_data = pd.read_csv(os.path.join(csv_directory, pilot_csv_name), sep=',')
reading_data = pd.read_csv(os.path.join(csv_directory, reading_csv_name), sep=',')
recurrence_data = pd.read_csv(os.path.join(recurrence_dir, recurrence_csv_name), sep=',')
...
def preprocess_image_medici(image_path, side, format, view):
    ...
    # Read the DICOM file
    try:
        current_mammogram = pydicom.dcmread(os.path.join(image_directory, image_path))
...
if __name__ == "__main__":
    patient_data = {}
    for index, row in tqdm(pilot_data.iterrows(), total=len(pilot_data), desc="Processing rows"):
        patient = row.Case
        ...
        if mlo_image == 'fail':
            ...
            print("failed reading", failed_dycom_reads[-1])
        if not sum(recurrence_data['TNO'] == patient):
            print(f"Patient {patient} does not have recurrence labels")
            missing_recurrence_data.append(patient)
            continue
        else:
            recurrence_row = recurrence_data[recurrence_data['TNO'] == patient]
            patient_data[patient]['recurrence'] = {
                'breastrec': recurrence_row['breastrec'].values[0],
                'distrfievent': recurrence_row['distrfievent'].values[0],
                'local': recurrence_row['local'].values[0],
                'Ipsbreast': recurrence_row['Ipsbreast'].values[0],
                'Contrabreast': recurrence_row['Contrabreast'].values[0],
            }
    ...
    for index, row in tqdm(reading_data.iterrows(), total=len(reading_data), desc="Processing rows"):
        patient = row.Case
        if patient not in patient_data:
            patient_data[patient] = {}
        side = row.Side
