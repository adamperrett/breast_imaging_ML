...
csv_name = 'MAI-VAS_readings_Increased_risk_images.csv'
save_name = 'BCAN-RAY_images.pth'

csv_data = pd.read_csv(os.path.join(csv_directory, csv_name), sep=',')

image_directory = 'W:\BCAN_ray'

failed_dycom_reads = []

def preprocess_image_medici(image_path, side, format, view, npy_file):
    """
    ...
    if ('GE' in Manufacturer or Manufacturer == 'KODAK' or Manufacturer == 'LORAD'):
    ...
    elif (Manufacturer == 'FUJIFILM Corporation'):
        ...
        print(f"Manufacturer {Manufacturer} not recognised")
...
if __name__ == "__main__":
    ...
    for base, dirnames, files in os.walk(image_directory):
        for file in files:
            full_path = os.path.join(base, file)
            # Skip npy files
            if '.npy' in file:
                patient = base.split('\\')[2]
                print("Found npy for", patient)
                npy_image = np.load(full_path)
                # dicom_image = pydicom.read_file(full_path[:-4])
                print("Processing image", full_path[:-4])
                if 'MLO' in full_path:
                    processed_image, Manufacturer = preprocess_image_medici(full_path[:-4], 'R', 'PRO', 'MLO', npy_image)
                else:
                    processed_image, Manufacturer = preprocess_image_medici(full_path[:-4], 'R', 'PRO', 'CC', npy_image)
                ...
                if 'GE' in Manufacturer:
                    manu = 'GE'
                elif 'HOLOGIC' in Manufacturer:
                    manu = 'HOLOGIC'
                else:
                    print('Unknown Manufacturer', Manufacturer)
                patient_data[patient][full_path[:-4]] = [processed_image, manu]
                plt.imshow(processed_image[0])
                plt.axis('off')  # Turn off axis labels
                if len(base.split('\\')) > 4:
                    print("This is a double dir patient", patient)
                    image_num = int(base.split('\\')[-1])
                else:
                    image_num = 0
                image_name = base.split('\\')[2] + '_' + file[:-4] + '_{}_{}'.format(image_num, manu)
                image_plotting_dir = 'C:\\Users\\adam_\\PycharmProjects\\breast_imaging_ML\\data_processing\\images\\'
                plt.savefig(image_plotting_dir + image_name + '.png')
            # else:
            #     continue
